...
- **SQLite registry**: `.data/baluarte.db` (gitignored). Schema source of truth: `.claude/skills/baluarte-remember/schema.sql`. Rebuild any time with:
  ...
  `baluarte-fetch-entities` also seeds two side stores on every run: project-level **Figma Variables** (from `/v1/files/{key}/variables/local`) become `properties` rows with `origin='Figma Variable'` and a stable `figma_variable_id`, and the **`pages`** table gets one row per file (`file_key` + `edited_at = lastModified`) with every just-saved entity linked into `pages_registry`. Files outside an Enterprise/Org plan return 403 on the Variables endpoint; the seeder logs and skips, the rest of the pipeline still runs.
