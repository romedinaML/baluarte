...
| `baluarte-remember` | The only skill that touches SQLite. Used internally by every other skill. | (operation-specific) | n/a — invoked programmatically. |
...
- **Never write SQL outside the `baluarte-remember` skill.** If a new operation is needed, add it to that skill's SKILL.md (and update `schema.sql` if the table needs to change).
